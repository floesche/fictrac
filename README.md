...
1. Download and install required build tools and dependencies:
    ...
    2. Linux only:
        1. Run the following from terminal to install necessary build tools and dependencies (this works for Ubuntu-22.04 and newer, otherwise you might also need `libavresample-dev`):
        ```
        sudo apt update
        sudo apt install gcc g++ git cmake curl unzip tar yasm pkg-config libgtk2.0-dev libavformat-dev libavcodec-dev libswscale-dev libopencv-dev
        ```
